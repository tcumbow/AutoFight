...
local function UnitCanBeHealed(unitTag)
	return (IsUnitInGroupSupportRange(unitTag) and not IsUnitDead(unitTag) and GetUnitType(unitTag) == 1 and not DoesUnitHaveResurrectPending(unitTag) and not IsUnitReincarnating(unitTag))
end
...
local function PreAttackAutoFight()
	...
	elseif SynergyName == "Purifying Blessing" then DoSynergy()
...
AutoFight[GIDEON] = function ()
	...
	elseif LowestGroupHealthPercentWithoutRegen()<80 and ActiveBar()==1 then UseAbility(2)
	elseif PreAttackAutoFight() then
	elseif UltimateReady() and TargetIsHostileNpc() then UseUltimate()
	elseif LowestGroupHealthPercentWithoutRegen()<90 and ActiveBar()==1 then WeaveAbility(2)
	-- elseif InMeleeRange and Magicka()>30 then WeaveAbility(4)
	-- elseif not IHave("Major Resolve") and Magicka()>40 then WeaveAbility(5)
	elseif SomeoneCouldUseRegen() and Magicka()>50 and ActiveBar()==1 then WeaveAbility(2)
